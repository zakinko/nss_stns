...
response_callback(void *buffer, size_t size, size_t nmemb, void *userp)
{
	...
	if (res->size + segsize + 1 > STNS_MAX_BUFFER_SIZE) {
		syslog(LOG_ERR, "nss_stns: response is too large");
		return 0;
	}
...
http_request(stns_conf_t *c, const char *path, stns_response_t *res)
{
	...
	if (result != CURLE_OK) {
		if (result == CURLE_COULDNT_CONNECT || result == CURLE_OPERATION_TIMEDOUT ||
			...
			rc = STNS_NG;
		syslog(LOG_ERR, "nss_stns: request to %s failed: %s", url, curl_easy_strerror(result));
	} else if (code >= 400) {
		res->size = 0;
		if (res->data != NULL)
			res->data[0] = '\0';
		if (code != STNS_HTTP_NOTFOUND)
			syslog(LOG_ERR, "nss_stns: request to %s failed with status %ld", url, code);
		rc = STNS_NG;
	} else {
		rc = STNS_OK;
	}
	...
	while ((ent = readdir(dp)) != NULL) {
...

		{
			...
			if ((sb.st_size > 0 && diff > c->cache_ttl) ||
				...
				if (unlink(buf) == -1)
					syslog(LOG_ERR, "nss_stns: cannot unlink %s: %s", buf, strerror(errno));
			}
		}
	}
	(void)closedir(dp);
...
stns_request(stns_conf_t *c, const char *path, stns_response_t *res)
{
	...
	if (c->query_wrapper != NULL) {
		...
		for (retry = c->request_retry; rc != STNS_OK && retry > 0; retry--) {
			...
			(void)sleep(1);
			syslog(LOG_NOTICE, "nss_stns: retrying request, %d attempts left", retry);
			rc = http_request(c, path, res);
		}
		if (rc == -1) {
			breaker_trip(c);
