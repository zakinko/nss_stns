 ...
 *
 * The lookup logic shared by every platform.  Everything here speaks NS_*
 * status codes (plus STNS_NS_ERANGE) and knows nothing about how nsdispatch(3)
 * lays out its arguments; that part lives in nss_stns.c.
 */
#include <errno.h>
...
static const char *
obj_str(JSON_Object *o, const char *key)
{
 ...
 */
static int
obj_int(JSON_Object *o, const char *key)
{
...
stns_fetch(stns_conf_t *c, const char *path, JSON_Value **rootp)
{
	...
	if (stns_request(c, path, &r) != STNS_OK) {
		free(r.data);
		return (r.status_code == STNS_HTTP_NOTFOUND) ? NS_NOTFOUND : NS_UNAVAIL;
	}

	root = json_parse_string(r.data);
	free(r.data);
	if (root == NULL) {
		syslog(LOG_ERR, "nss_stns: cannot parse the response to /%s", path);
		return NS_UNAVAIL;
	}
	if (json_value_get_array(root) == NULL) {
		syslog(LOG_ERR, "nss_stns: unexpected response shape for /%s", path);
...
stns_pw_by_name(stns_conf_t *c, const char *name, struct passwd *pwd, char *buf, size_t buflen, int *errnop)
{
	...
	if ((rv = stns_fetch(c, path, &root)) != NS_SUCCESS)
		return rv;
	...
	for (i = 0; i < n; i++) {
		...
		if (o == NULL)
			continue;
		if ((found = obj_str(o, "name")) == NULL || strcmp(found, name) != 0)
			continue;
    ...
    int *errnop, int (*fill)(JSON_Object *, stns_conf_t *, void *, char *, size_t, int *))
{
	...
	if (*entries == NULL && (rv = ent_start(c, path, entries, index)) != NS_SUCCESS)
		return rv;
...
stns_gr_membership(stns_conf_t *c, const char *uname, gid_t agroup, gid_t *groups, int maxgrp, int *groupc)
{
	...
	if ((rv = stns_fetch(c, "groups", &root)) != NS_SUCCESS)
		return rv;
	...
	/*
	 * Report NS_NOTFOUND even on success: getgroupmembership(3) merges the
	 * result of every source, so claiming NS_SUCCESS would stop the search
	 * and drop the local groups from /etc/group.
	 */
	return NS_NOTFOUND;
}
