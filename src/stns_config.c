...
conf_str(toml_table_t *tab, const char *key, char **dst, const char *def, const char *filename)
{
	...
	if (tab != NULL) {
		...
		if (toml_key_exists(tab, key))
			syslog(LOG_ERR, "nss_stns: %s: key '%s' is not a string", filename, key);
	}
	*dst = stns_strdup(def);
}
	...
	*dst = def;
	...
	if (d.ok)
		*dst = (int)d.u.i;
		...
		syslog(LOG_ERR, "nss_stns: %s: key '%s' is not an integer", filename, key);
}
	...
	*dst = def;
	...
	if (d.ok)
		*dst = d.u.b;
		...
		syslog(LOG_ERR, "nss_stns: %s: key '%s' is not a boolean", filename, key);
}
...
load_http_headers(toml_table_t *tab, stns_conf_t *c, const char *filename)
{
	...
	while (n < STNS_MAXBUF) {
		...
		if (!d.ok) {
			syslog(LOG_ERR, "nss_stns: %s: http_headers.%s is not a string", filename, key);
			break;
		}
		value = d.u.s;
...
count_unknown(toml_table_t *tab, const char *const *known, const char *table, const char *filename, int *warned)
{
	...
	for (i = 0; (key = toml_key_in(tab, i)) != NULL; i++) {
		...
		if (*warned)
			continue;
		if (table != NULL)
			syslog(LOG_NOTICE, "nss_stns: %s: unknown key '%s' in [%s], ignored", filename, key,
			    table);
		else
			syslog(LOG_NOTICE, "nss_stns: %s: unknown key '%s', ignored", filename, key);
	}
	return n;
}
...
stns_load_config(const char *filename, stns_conf_t *c)
{
	...
	if ((fp = fopen(filename, "r")) == NULL) {
		syslog(LOG_ERR, "nss_stns: cannot open %s: %s", filename, strerror(errno));
		return STNS_NG;
	}
	...
	(void)fclose(fp);
	if (tab == NULL) {
		syslog(LOG_ERR, "nss_stns: %s: %s", filename, errbuf);
		return STNS_NG;
	}
	...
	if (toml_key_exists(tab, "api_endpoint") == 0) {
		...
		if (!warned) {
			warned = 1;
			syslog(LOG_NOTICE, "nss_stns: %s: no api_endpoint, using %s", filename,
