...
STAGE=${STAGE:-/tmp/nss_stns_stage}
...
# NSSLIBDIR is pinned inside PREFIX exactly as every package does it, so that
# the /usr/lib symlink - which no package manager will stage - is left to the
# package's own install hooks.
cd "$SRCDIR"
make DESTDIR="$STAGE" PREFIX="$PREFIX" NSSLIBDIR="$PREFIX/lib" install >/dev/null
