 ...
 *
 * Unit tests for the parts of nss_stns that do not need a server: config
...
test_fill_passwd(void)
{
	...
	CHECK(stns_fill_passwd(json_value_get_object(v), &c, &pwd, buf, sizeof(buf), &errnop) == NS_SUCCESS);
...
test_fill_passwd_bad_entry(void)
{
	...
	CHECK(stns_fill_passwd(json_value_get_object(v), &c, &pwd, buf, sizeof(buf), &errnop) == NS_NOTFOUND);
	json_value_free(v);
...
test_fill_group(void)
{
	...
	CHECK(stns_fill_group(json_value_get_object(v), &c, &grp, buf, sizeof(buf), &errnop) == NS_SUCCESS);
